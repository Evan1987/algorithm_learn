...
def linear(x: torch.Tensor, weight: torch.Tensor, bias: Optional[torch.Tensor] = None) -> torch.Tensor:
    ...
    Args:
        ...
        weight (torch.Tensor): The weight tensor. It may be quantized and requires dequantization for certain cases.
    ...
    Notes:
        - If `weight` is quantized (e.g., `element_size() > 1`), a dequantized version is used for computation.
...
class Linear(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = False, dtype=None):
        ...
        if bias:
            self.bias: torch.Tensor = nn.Parameter(torch.empty(self.out_features))
        else:
            self.register_parameter("bias", None)

    def forward(self, x: torch.Tensor) -> torch.Tennsor:
        """
...
def precompute_freq_cis(args: ModelArgs) -> torch.Tensor:
    """
    ...
    def find_correction_dim(num_rotations: float, dim: int, base: int, max_seq_len: int) -> float:
        """
    ...
    def find_correction_range(low_rot: float, high_rot: float, dim: int, base: float, max_seq_len: int) -> Tuple[
        int, int]:
        """
    ...
    def linear_ramp_factor(min: int, max: int, dim: int) -> torch.Tensor:
        """
...
def apply_rotary_emb(x: torch.Tensor, freq_cis: torch.Tensor) -> torch.Tensor:
    ...
    Args:
        ...
        freq_cis (torch.Tensor): Precomputed complex exponential values for positional embeddings.
    ...
    def __init__(self, args: ModelArgs):
        ...
        def forward(self, x: torch.Tensor, start_pos: int, freq_cis: torch.Tensor, mask: Optional[torch.Tensor]):
            ...
            Args:
                ...
                freq_cis (torch.Tensor): Precomputed complex exponential values for rotary embeddings.
...
class Gate(nn.Module):
    ...
    Attributes:
        ...
        top_k (int): Number of top experts activated for each input.
        n_groups (int): Number of groups for routing.
        top_k_groups (int): Number of groups to route inputs to.
...
class Transformer(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        self.register_buffer("freq_cis", precompute_freq_cis(args), persistent=False)
