...
from ragen.trajectory_transformation import DecisionMaking
from typing import *
    ...
    for step in range(max_steps):
        ...
        for idx, (env, action, info, policy_input, action_full, traj) in enumerate(zip(env_instances, actions, history_info, policy_inputs, actions_full, trajectories)):
            if not isinstance(action, list):
                action = [action]
            if not isinstance(action_full, list):
                action_full = [action_full]
            ...
            for a, a_full, pinput in zip(action, action_full, policy_input):
                if step >= max_steps:
                    break
                # env_instances, history_info, trajectories are updated each step
                env, info, traj, done = _update_env(env, step, info, a, pinput, a_full, traj)
