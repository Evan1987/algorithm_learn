...
class AbstractNgramFilter(abc.ABC):
...
class NgramFilterWrapper(AbstractNgramFilter):

    def __init__(self, ngram_filters: List[AbstractNgramFilter] = None):
        super().__init__()
...
class DefaultNgramFilter(AbstractNgramFilter):

    def apply(self, ngram: List[str], **kwargs) -> bool:
...
class NgramFilteringExtractor(AbstractCorpusReadCallback):
    ...
    def __init__(self, ngram_filters: List[AbstractNgramFilter] = None, use_default_ngram_filters: bool = True):
        super().__init__()
...
class NgramsExtractor(NgramFilteringExtractor, AbstractFeatureExtractor):
    ...
    def pmi_of(self, phrase: str) -> float:
...
class IDFExtractor(NgramFilteringExtractor, AbstractFeatureExtractor):
    ...
    def doc_freq_of(self, phrase: str) -> int:
        """Get doc freq of phrase."""
...
class EntropyExtractor(NgramFilteringExtractor, AbstractFeatureExtractor):
    ...
    def left_entropy_of(self, phrase: str) -> float:
    ...
    def right_entropy_of(self, phrase: str) -> float:
